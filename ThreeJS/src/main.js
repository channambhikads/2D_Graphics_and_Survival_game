...
class HackNSlashDemo {
  ...
  _Initialize() {
    ...
    this._threejs.shadowMap.type = THREE.PCFSoftShadowMap; // or BasicShadowMap for faster
    ...
    light.shadow.mapSize.width = 512; // default is 1024 or higher
...
const stoneTexture = textureLoader.load('./resources/textures/stone.jpg');

stoneTexture.wrapS = THREE.RepeatWrapping;
stoneTexture.wrapT = THREE.RepeatWrapping;
stoneTexture.repeat.set(100, 50); // Increase for more tiles, decrease for fewer
...
this._scene.add(plane);
  ...
  _LoadPlayer() {
    ...
    const girl = new entity.Entity('girl');
    ...
    // Add this HealthComponent so she can be attacked
    girl.AddComponent(new health_component.HealthComponent({
        health: 50,
        maxHealth: 50,
        strength: 1,
        wisdomness: 1,
    ...
    girl.AddComponent(new health_bar.HealthBar({
        parent: this._scene,
        camera: this._camera,
    }));
    ...
    for (let i = 0; i < 50; ++i) {
      ...
      npc.SetPosition(new THREE.Vector3(
          ...
          0,
          (Math.random() * 2 - 1) * 500));
      this._entityManager.Add(npc); // <-- Add to manager LAST!
    }
  }
