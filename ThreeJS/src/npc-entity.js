...
export const npc_entity = (() => {
  ...
  class NPCController extends entity.Component {
    ...
    _Init(params) {
      this._params = params;
      this._decceleration = new THREE.Vector3(-1.0, -0.1, -10.0);
      this._acceleration = new THREE.Vector3(20.0, 0.25, 80.0);
    ...
    InitComponent() {
      this._entityManager = this._parent._manager;
      console.log('NPCController InitComponent', this._entityManager);
  ...
  if (player && player.GetComponent('HealthComponent')?.IsAlive()) {
    targets.push(player);
  }

  const girl = this._entityManager.Get('girl');
  if (girl && girl.GetComponent('HealthComponent')?.IsAlive()) {
    targets.push(girl);
  }
  ...
  for (let t of targets) {
    const dist = t._position.distanceTo(this._parent._position);
    if (dist < minDist) {
      minDist = dist;
      nearest = t;
    }
  ...
  return nearest;
    ...
    _UpdateAI(timeInSeconds) {
      ...
      dir.subVectors(target._position, this._parent._position);
    ...
    _OnAIWalk(timeInSeconds) {
      // Find nearest target (player or girl)
      let targets = [];
      const player = this._entityManager.Get('player');
      if (player && player.GetComponent('HealthComponent') && player.GetComponent('HealthComponent').IsAlive()) {
        targets.push(player);
      }
      const girl = this._entityManager.Get('girl');
      if (girl && girl.GetComponent('HealthComponent') && girl.GetComponent('HealthComponent').IsAlive()) {
        targets.push(girl);
      }
      ...
      let minDist = Infinity;
      for (let t of targets) {
        let dist = t._position.distanceTo(this._parent._position);
        if (dist < minDist) {
          minDist = dist;
          nearest = t;
        }
      ...
      const dirToTarget = nearest._position.clone().sub(this._parent._position);
      dirToTarget.y = 0.0;
      ...
      // After moving the monster, check distance to nearest target
      const attackDistance = 2.0; // Adjust as needed
      const distanceToTarget = nearest._position.distanceTo(this._parent._position);

      if (distanceToTarget < attackDistance) {
        // Switch to attack state
        this._stateMachine.SetState('attack');
      } else {
        // Switch to walk state if not attacking
        this._stateMachine.SetState('walk');
      }
    }
    ...
    Update(timeInSeconds) {
      ...
      if (nearest) {
        // Your movement logic here, e.g.:
        // this._MoveTowards(nearest._position);
      }
    }
  };
